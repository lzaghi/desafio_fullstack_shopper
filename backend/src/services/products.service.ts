...
export default class ProductsService implements IProductsService {
  ...
  async validateProducts(inputProducts: IInputProduct[]): Promise<IValidatedProduct[]> {
    ...
    const invalidProducts: IValidatedProduct[] = [];
    
    const validatedProcuts = inputProducts
      .sort((a, b) => +a.product_code - +b.product_code)
      .map((product) => {
        ...
        if(!product.product_code || !product.new_price) {
          ...
          invalidProducts.push(invalidInputProduct);
        ...
        if(!this.validSalePrice(+product.new_price, +dbProduct.cost_price)) {
          ...
          invalidProducts.push(invalidSalePriceProduct);
        ...
        if(!this.validReadjustment(+product.new_price, dbProduct)) {
          ...
          invalidProducts.push(invalidReadjustmentProduct);
          return invalidReadjustmentProduct;
        }

        if(!this.validPrackProducts(dbProduct, invalidProducts)) {
          const invalidPackProductsProduct = {
            ...validProduct,
            error: 'Produtos associados a esse pacote apresentam reajustes inválidos'
          }
          return invalidPackProductsProduct;
        }
  ...
  private validPrackProducts(dbProduct: IProduct, invalidProducts: IValidatedProduct[]): boolean {
    if(dbProduct.hasProducts!.length) {
      for(const product of dbProduct.hasProducts!) {
        const packProduct = invalidProducts.find((p) => +p.product_code === +product.product_id);
        if(packProduct) {
          return false;
        }
      }
